...
| WP08 Conditioning | Additive scene handle; Factory Render/Mask registration; shared RGB/geometry camera capture; depth/normal/RGB24 shaders; metric Float32 storage; immutable snapshot and capture manifests; staged per-shot uploads; exact cached reuse; entity masks with grow/feather; Export progress/cancel/selection keys; backend camera/revision/asset checks | GPU pass acceptance, real downstream list integration, supplied mesh proxies, collection/semantic masks, orthographic and alternate normal/depth profiles, downloadable conditioning ZIP |
...
| WP07, WP12, WP15–19, WP21–22 | Existing capabilities retained; remaining work as specified below | Unified virtualized hierarchy, scatter/splines, advanced lighting/materials/assets, large scenes, variants/render host and release gates |
...
- The original node output remains slot 0 `preview`, IMAGE list, current view followed by saved cameras. Slot 1 is an opaque `VNCCS_FACTORY_SCENE` handle. Factory Render emits six aligned lists plus a non-list capture handle; Factory Mask selects integer entity IDs. Conditioning uses separate version-1 manifests outside scene JSON, so existing scene-11/12 documents retain their schema semantics. RGB uses current preview lighting; deferred shadows can leak through walls. Gaussian geometry requires explicit coarse-box approval; no exact splat depth is claimed.
- `api/factory3d_conditioning.py` persists snapshot documents with scene/asset hashes, allocates stable non-reused RGB24 IDs, validates per-shot camera matrices against the frozen pose, decodes packed depth to Float32 meters and atomically publishes complete captures. Cache identity includes snapshot and render settings/build. Old handles reuse exact cached captures or fail when source content changed. Published captures are not evicted by later captures. Staging expires after ten minutes; terminal staging metadata expires after one day on subsequent requests.
- `conditioning.mjs` bakes mesh transforms, mirrored winding and supported skin/morph poses into owned capture geometries. Dedicated unlit passes share the exact RGB projection and explicit material-cutout policy, use a non-MSAA nearest RGBA8 target and restore renderer state in `finally`. Binary coverage has foreground=1 and excludes sky. Current limits are 2048 per axis, 32 megapixels per request and 20000 evaluated meshes/instances. Current fresh capture is perspective-only and requires the 3D workspace view. Full float targets, antialiased coverage, supplied proxy meshes, independent render host and GPU visual acceptance remain outstanding.

Earlier validation: 64 focused JS geometry/transaction/frontend/light tests; 9 Python node tests; 2 Python migration/storage tests; 1 native-package test covering both scene and object packages; shared numeric normalization cases. The conditioning iteration adds CPU geometry/ID/orientation tests and Python capture/storage/camera/mask/list tests, including the real storage adapter, stale source rejection and corrupt-part checks. Required security scanner/integrity checks remain unchanged. No ComfyUI interaction, model generation, GPU benchmarks or visual acceptance is claimed. See the remaining work-package table and release gates before treating this as a release.
