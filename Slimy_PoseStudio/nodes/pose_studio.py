...
def _ensure_data_loaded():
    ...
    # Reset cache fully before loading to prevent partial/corrupted state
    POSE_STUDIO_CACHE['base_mesh'] = None
    POSE_STUDIO_CACHE['targets'] = None
    POSE_STUDIO_CACHE['parser'] = None
    POSE_STUDIO_CACHE['skeleton'] = None
...
class VNCCS_PoseStudio:
    ...
    def generate(
        ...
        # Parse pose data (widget values - settings only, no images)
        try:
        ...
        # Request a fresh capture from the frontend right now
        if unique_id:
            try:
                from server import PromptServer
                import time

                # Create a one-shot event for this request
                import threading
                result_holder = {}
                event = threading.Event()

                # Register a temporary capture receiver
                from .. import VNCCS_PENDING_CAPTURES
                VNCCS_PENDING_CAPTURES[str(unique_id)] = (result_holder, event)

                # Ask JS to capture and POST back
                PromptServer.instance.send_sync("vnccs_req_capture", {"node_id": unique_id})

                # Wait up to 5s
                if event.wait(timeout=5.0):
                    captured = result_holder.get("data", {})
                    data["captured_images"] = captured.get("captured_images", [])
                    data["lighting_prompts"] = captured.get("lighting_prompts", [])
                    print(f"[Slimy_VNCCS] Capture OK (node {unique_id})")
                else:
                    print(f"[Slimy_VNCCS] Capture timeout (node {unique_id})")

                # Clean up
                VNCCS_PENDING_CAPTURES.pop(str(unique_id), None)

            except Exception as e:
                print(f"[Slimy_VNCCS] Capture error: {e}")
...
# Register Hand Pose Library API routes
try:
    from .vnccs_hand_pose_library_api import register_hand_pose_library_routes
    register_hand_pose_library_routes()
except Exception as e:
    print(f"[VNCCS] Failed to register hand pose library routes: {e}")
