...
const SEGMENT_TO_BONE = [
    ...
    // Right leg
    { parent: "r_hip",  child: "r_knee",  mhBone: "thigh_r",  parentSegment: null },
    { parent: "r_knee", child: "r_ankle", mhBone: "calf_r",   parentSegment: { parent: "r_hip", child: "r_knee" } },

    // Left leg
    { parent: "l_hip",  child: "l_knee",  mhBone: "thigh_l",  parentSegment: null },
...
export function convertOpenPoseToPose(parsed, viewer) {
    ...
    // --- Process legs separately (need X and Z rotation) ---
    const legDebug = _processLegs(joints, viewer, bones);
    ...
    for (const seg of SEGMENT_TO_BONE) {
        ...
        // Skip legs — handled by _processLegs
        if (['thigh_r', 'thigh_l', 'calf_r', 'calf_l'].includes(seg.mhBone)) continue;
        ...
        // clavicle and upperarm use original sign, lowerarm and below use negated Z
        const useOriginalSign = seg.mhBone.startsWith('clavicle') || seg.mhBone.startsWith('upperarm');
        const zSign = useOriginalSign ? 1 : -1;
        bones[seg.mhBone] = [deltaDeg * sinB, 0, deltaDeg * cosB * zSign];
    }
    ...
    return { bones, modelRotation, cameraYRotation: bodyYRot, legDebug };
}
...
/**
 * Process legs: compute X (sagittal/front-back) and Z (frontal/left-right) rotations.
 * Tpose legs point straight down (-Y), so Z-only rotation won't move them.
 */
function _processLegs(joints, viewer, bones) {
    const sides = [
        { hip: 'r_hip', knee: 'r_knee', ankle: 'r_ankle', thigh: 'thigh_r', calf: 'calf_r' },
        { hip: 'l_hip', knee: 'l_knee', ankle: 'l_ankle', thigh: 'thigh_l', calf: 'calf_l' },
    ];
    const debugLines = [];

    for (const s of sides) {
        const hipKp  = joints[s.hip];
        const kneeKp = joints[s.knee];
        const ankleKp = joints[s.ankle];

        if (!hipKp || !kneeKp || hipKp.c < CONFIDENCE_THRESHOLD || kneeKp.c < CONFIDENCE_THRESHOLD) {
            debugLines.push(`${s.thigh}: SKIPPED (no keypoint)`);
            continue;
        }

        const dx = kneeKp.x - hipKp.x;
        const dy = -(kneeKp.y - hipKp.y);

        // Log bone local axes
        const thighBone = viewer.bones[s.thigh];
        if (thighBone) {
            const xAxis = new viewer.THREE.Vector3(1,0,0).applyQuaternion(thighBone.getWorldQuaternion(new viewer.THREE.Quaternion()));
            const yAxis = new viewer.THREE.Vector3(0,1,0).applyQuaternion(thighBone.getWorldQuaternion(new viewer.THREE.Quaternion()));
            const zAxis = new viewer.THREE.Vector3(0,0,1).applyQuaternion(thighBone.getWorldQuaternion(new viewer.THREE.Quaternion()));
            debugLines.push(`${s.thigh} localX-world: (${xAxis.x.toFixed(2)},${xAxis.y.toFixed(2)},${xAxis.z.toFixed(2)})`);
            debugLines.push(`${s.thigh} localY-world: (${yAxis.x.toFixed(2)},${yAxis.y.toFixed(2)},${yAxis.z.toFixed(2)})`);
            debugLines.push(`${s.thigh} localZ-world: (${zAxis.x.toFixed(2)},${zAxis.y.toFixed(2)},${zAxis.z.toFixed(2)})`);
        }

        const restAngle = _restAngle(viewer, s.thigh) ?? -Math.PI / 2;
        const segAngle = Math.atan2(dy, dx);
        const delta = _normalizeAngle(segAngle - restAngle);

        // thigh localY points along the bone (leg direction)
        // thigh localX points roughly in world XY plane → rotation around localX = lateral swing
        // Project delta onto localX and localZ of the bone
        // In T-pose: localX=(0.74,-0.67,0), localZ=(0,0,1)
        // Lateral swing (world X movement) maps to localX rotation
        // The 2D angle delta is in the world XY frontal plane
        // Map: rotX = delta * sign based on side, rotZ = 0
        const sign = s.thigh.endsWith('_r') ? -1 : 1;
        const rotX = delta * RAD2DEG * sign;

        debugLines.push(`${s.thigh}: segAngle=${(segAngle*180/Math.PI).toFixed(1)}° restAngle=${(restAngle*180/Math.PI).toFixed(1)}° rotX=${rotX.toFixed(1)}°`);

        bones[s.thigh] = [rotX, 0, 0];

        if (ankleKp && ankleKp.c >= CONFIDENCE_THRESHOLD) {
            const dx2 = ankleKp.x - kneeKp.x;
            const dy2 = -(ankleKp.y - kneeKp.y);
            const calfAngle = Math.atan2(dy2, dx2);
            const calfRestAngle = _restAngle(viewer, s.calf) ?? restAngle;
            const calfDeltaZ = _normalizeAngle(calfAngle - segAngle - (calfRestAngle - restAngle)) * RAD2DEG;
            debugLines.push(`${s.calf}: calfAngle=${(calfAngle*180/Math.PI).toFixed(1)}° calfDeltaZ=${calfDeltaZ.toFixed(1)}°`);
            bones[s.calf] = [0, 0, calfDeltaZ];
        }
    }
    return debugLines;
}
