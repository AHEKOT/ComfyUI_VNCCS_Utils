...
async def upload_pose_sync(request):
    """POST /vnccs/pose_sync/upload_capture - Stores synchronized capture in memory cache."""
    try:
        data = await request.json()
        ...
        # Store directly in the in-memory LRU cache — no temp files, no race conditions
        from .. import VNCCS_CAPTURE_CACHE, _CAPTURE_CACHE_MAX
        VNCCS_CAPTURE_CACHE[node_id] = {
            "captured_images": data.get("captured_images", []),
            "lighting_prompts": data.get("lighting_prompts", []),
            "pose_data": data,  # store full payload so generate() can use it
        }
        # LRU eviction
