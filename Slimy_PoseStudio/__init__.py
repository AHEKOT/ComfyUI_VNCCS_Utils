...
from .nodes.rtmw_pose import VNCCS_RTMWPose
from .nodes.metrabs_pose import VNCCS_MeTRAbs3DPose
from .nodes.hmr2_pose import VNCCS_HMR2Pose
...
NODE_CLASS_MAPPINGS = {
    ...
    "VNCCS_RTMWPose": VNCCS_RTMWPose,
    "VNCCS_MeTRAbs3DPose": VNCCS_MeTRAbs3DPose,
    "VNCCS_HMR2Pose": VNCCS_HMR2Pose,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "VNCCS_PoseStudio": "Slimy_PoseStudio",
    "VNCCS_RTMWPose": "Slimy_RTMWPose",
    "VNCCS_MeTRAbs3DPose": "Slimy_MeTRAbs3DPose",
    "VNCCS_HMR2Pose": "Slimy_HMR2Pose",
}
...
# Register Hand Pose Library API
def _vnccs_register_hand_pose_library():
    try:
        from server import PromptServer
        from .api.hand_pose_library import register_routes
        register_routes(PromptServer.instance.app)
    except Exception as e:
        print(f"[VNCCS] Failed to register Hand Pose Library API: {e}")

_vnccs_register_hand_pose_library()



# === Pose Studio Capture (one-shot, no cache) ===
VNCCS_PENDING_CAPTURES = {}  # node_id -> (result_holder, threading.Event)
...
def _vnccs_register_capture_cache():
    ...
    @PromptServer.instance.routes.post("/vnccs/capture_result")
    async def vnccs_capture_result(request):
        try:
            data = await request.json()
            node_id = str(data.get("node_id", ""))
            pending = VNCCS_PENDING_CAPTURES.get(node_id)
            if pending:
                result_holder, event = pending
