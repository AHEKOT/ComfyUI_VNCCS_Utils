...
class PoseViewer extends PoseViewerCore {
    constructor(canvas) {
        super(canvas, { syncMode: "end" });

        // Preserve legacy fixed viewport defaults used by the existing widget layout.
        this.width = 500;
        this.height = 500;

        // Legacy callback contract used by PoseStudioWidget.
        this.syncCallback = null;

        // Deferred camera snap requested before initialization is ready.
    ...
    async init() {
        try {
            ...
            // Apply any deferred camera snap after data/lights/background are ready.
            if (this.pendingSnap) {
                const { width, height, zoom, offsetX, offsetY } = this.pendingSnap;
                this.pendingSnap = null;
                this.snapToCaptureCamera(width, height, zoom, offsetX, offsetY);
            }
    ...
    setupScene() {
        ...
        // Flag: when true, capture() uses stored camera state instead of recalculating from exportParams
        this.viewportCameraOverride = false;
    ...
    handlePointerDown(e) {
        ...
        // --- If nothing hit ---
        // If hovering over gizmo axis, let TransformControls handle it without deselecting
        ...
        if (this.lastClickTime && (now - this.lastClickTime < DOUBLE_CLICK_THRESHOLD)) {
    ...
    snapToCaptureCamera(width, height, zoom = 1.0, offsetX = 0, offsetY = 0) {
        if (!this.initialized || !this.captureCamera || !this.camera || !this.orbit) {
            this.pendingSnap = { width, height, zoom, offsetX, offsetY };
            return;
        }

        // Switching to param-driven camera control should disable viewport override.
        this.viewportCameraOverride = false;
    ...
    setCaptureToViewport() {
        if (!this.initialized || !this.camera || !this.captureCamera) return;

        // Copy all viewport camera properties directly to capture camera
        this.captureCamera.position.copy(this.camera.position);
        this.captureCamera.quaternion.copy(this.camera.quaternion);
        this.captureCamera.zoom = this.camera.zoom;
        this.captureCamera.fov = this.camera.fov;
        this.captureCamera.aspect = this.camera.aspect;
        this.captureCamera.near = this.camera.near;
        this.captureCamera.far = this.camera.far;
        this.captureCamera.updateProjectionMatrix();

        // Update reference plane (child of captureCamera, so position is in camera-local space)
        if (this.refPlane) {
            const planeDist = 95;
            const vFOV = (this.captureCamera.fov * Math.PI) / 180;
            const h = 2 * planeDist * Math.tan(vFOV / 2) / Math.max(0.1, this.captureCamera.zoom);
            const w = h * this.captureCamera.aspect;
        ...
        // Update capture frame (world-space visual indicator)
        if (this.captureFrame) {
            const target = this.orbit.target;
            const dist = this.captureCamera.position.distanceTo(target);
            const vFOV = (this.captureCamera.fov * Math.PI) / 180;
            const h = 2 * dist * Math.tan(vFOV / 2) / Math.max(0.1, this.captureCamera.zoom);
            const w = h * this.captureCamera.aspect;
        ...
        // Set flag so capture() skips updateCaptureCamera() override
        this.viewportCameraOverride = true;
    ...
    capture(width, height, zoom, bgColor, offsetX = 0, offsetY = 0) {
        ...
        // Ensure camera is setup (skip if user set capture cam to viewport manually)
        if (!this.viewportCameraOverride) {
            this.updateCaptureCamera(width, height, zoom, offsetX, offsetY);
        } else {
            // Still update aspect ratio in case output dimensions differ
            this.captureCamera.aspect = width / height;
            this.captureCamera.updateProjectionMatrix();
        }
...
class PoseStudioWidget {
    constructor(node) {
        ...
        this.exportParams = {
            ...
            cam_offset_x: 0,
            cam_offset_y: 0,
            capture_mode: "params", // params | viewport
            cam_viewport_state: null, // {pos:[x,y,z], quat:[x,y,z,w], target:[x,y,z], zoom,fov}
            output_mode: "LIST",
            grid_columns: 2,
        ...
        slider.addEventListener("input", () => {
            ...
            if (isExport) {
                ...
                if (isCamParam && this.viewer) {
                    this.exportParams.capture_mode = "params";
                    this.exportParams.cam_viewport_state = null;
    ...
    createCameraRadar(section) {
        ...
        recenterBtn.onclick = () => {
            // Clear viewport override so camera returns to export param control
            if (this.viewer) this.viewer.viewportCameraOverride = false;
            this.exportParams.capture_mode = "params";
            this.exportParams.cam_viewport_state = null;
        ...
        // Set Capture Camera to Frame Button
        const setCaptureBtn = document.createElement("button");
        setCaptureBtn.className = "vnccs-ps-btn";
        setCaptureBtn.style.marginTop = "4px";
        setCaptureBtn.style.width = "100%";
        setCaptureBtn.innerHTML = '<span class="vnccs-ps-btn-icon">📷</span> Capture Frame';
        
        setCaptureBtn.onclick = (e) => {
            e.preventDefault();
            e.stopPropagation();
            
            if (!this.viewer) return;
            
            const viewportCam = this.viewer.camera;
            const meshCenter = this.viewer.meshCenter || { x: 0, y: 10, z: 0 };
            
            // Update export params so the radar preview reflects the viewport position
            this.exportParams.cam_zoom = viewportCam.zoom;
            this.exportParams.cam_offset_x = meshCenter.x - viewportCam.position.x;
            this.exportParams.cam_offset_y = meshCenter.y - viewportCam.position.y;
            this.exportParams.capture_mode = "viewport";
            
            // Sync capture camera to current viewport view
            this.viewer.setCaptureToViewport();
            this.exportParams.cam_viewport_state = this.captureCurrentViewportState();
            
            // Redraw the radar display
            draw();
        ...
        wrap.appendChild(setCaptureBtn);
    ...
    captureCurrentViewportState() {
        if (!this.viewer || !this.viewer.camera || !this.viewer.orbit) return null;

        const cam = this.viewer.camera;
        const target = this.viewer.orbit.target;
        return {
    ...
    applyViewportState(state) {
        if (!state || !this.viewer || !this.viewer.camera || !this.viewer.orbit) return false;

        try {
            const cam = this.viewer.camera;
            cam.position.set(state.pos[0], state.pos[1], state.pos[2]);
            cam.quaternion.set(state.quat[0], state.quat[1], state.quat[2], state.quat[3]);
            cam.zoom = state.zoom ?? cam.zoom;
            cam.fov = state.fov ?? cam.fov;
            cam.updateProjectionMatrix();

            this.viewer.orbit.target.set(state.target[0], state.target[1], state.target[2]);
            this.viewer.orbit.update();

            this.viewer.setCaptureToViewport();
            return true;
        } catch (e) {
            console.warn("[VNCCS PoseStudio] Failed to apply viewport state:", e);
            return false;
        }
    }
    ...
    loadModel(showOverlay = true) {
        ...
        return api.fetchApi("/vnccs/character_studio/update_preview", {
            ...
            if (this.viewer) {
                // First load should center correctly; subsequent loads keep camera.
                this.viewer.loadData(d, this.hasLoadedModelOnce);

                // Apply lighting configuration
                this.viewer.updateLights(this.lightParams);

                // FORCE camera sync on every model change using selected capture mode
                if (this.exportParams.capture_mode === "viewport") {
                    if (!this.applyViewportState(this.exportParams.cam_viewport_state)) {
                        this.viewer.setCaptureToViewport();
                    }
                } else {
                    this.viewer.snapToCaptureCamera(
    ...
    syncToNode(fullCapture = false) {
        ...
        if (this.viewer && this.viewer.initialized) {
            ...
            // Keep capture camera synchronized with configured mode before capture.
            if (this.exportParams.capture_mode === "viewport") {
                this.exportParams.cam_viewport_state = this.captureCurrentViewportState();
                this.viewer.setCaptureToViewport();
            } else {
                this.viewer.viewportCameraOverride = false;
            }
    ...
    loadFromNode() {
        ...
        try {
            ...
            // Do not auto-load model here.
            // Startup sequencing (onNodeCreated/onConfigure) is responsible for a single authoritative load.
...
app.registerExtension({
            ...
            if (node && node.studioWidget) {
                try {
                    ...
                    if (node.studioWidget.viewer) {
                        ...
                        if (node.studioWidget.exportParams.capture_mode === "viewport") {
                            if (!node.studioWidget.applyViewportState(node.studioWidget.exportParams.cam_viewport_state)) {
                                node.studioWidget.viewer.setCaptureToViewport();
                            }
                        } else {
                            node.studioWidget.viewer.snapToCaptureCamera(
                                node.studioWidget.exportParams.view_width,
                                node.studioWidget.exportParams.view_height,
                                node.studioWidget.exportParams.cam_zoom || 1.0,
                                node.studioWidget.exportParams.cam_offset_x || 0,
                                node.studioWidget.exportParams.cam_offset_y || 0
                            );
                        }
                    }
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, _app) {
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            // Load model after initialization (new/empty node path only).
            setTimeout(() => {
                const poseDataWidget = this.widgets?.find(w => w.name === "pose_data");
                const hasSavedPoseData = !!(poseDataWidget && poseDataWidget.value);

                // If this node is being restored from workflow, onConfigure will handle authoritative load.
                if (hasSavedPoseData) {
        ...
        nodeType.prototype.onConfigure = function (info) {
            ...
            if (this.studioWidget) {
                setTimeout(() => {
                    ...
                    // Fresh configure load should treat this as startup load path.
                    this.studioWidget.hasLoadedModelOnce = false;
                    this.studioWidget.loadModel().then(() => {
                        if (this.studioWidget.viewer) {
                            if (this.studioWidget.exportParams.capture_mode === "viewport") {
                                if (!this.studioWidget.applyViewportState(this.studioWidget.exportParams.cam_viewport_state)) {
                                    this.studioWidget.viewer.setCaptureToViewport();
                                }
                            } else {
                                this.studioWidget.viewer.snapToCaptureCamera(
                                    this.studioWidget.exportParams.view_width,
